...
def _summarize_errors(
    ...
    *,
    ...
    finite_absolute = absolute.to_numpy(dtype=float).ravel()
    finite_absolute = finite_absolute[np.isfinite(finite_absolute)]
    mean_boundary_error = (
        float(finite_absolute.mean()) if finite_absolute.size else np.nan
    )
    p90_boundary_error = (
        float(np.quantile(finite_absolute, 0.90))
    ...
    return {
        ...
        "mean_boundary_mae_min": mean_boundary_error,
        "p90_boundary_abs_error_min": p90_boundary_error,
...
def _resolve_benchmark_path(comparison_run_dir: Path) -> Path:
    outputs = Path(comparison_run_dir) / "outputs"
    ...
    raise FileNotFoundError(
        "Boundary completion requires validation_benchmark_detail.csv or "
        f"validation_predictions.csv under {outputs}"
    )
...
def build_error_table(
    ...
    benchmark_path = _resolve_benchmark_path(comparison_run_dir)
    if benchmark_path.name == "validation_benchmark_detail.csv":
        benchmark = pd.read_csv(benchmark_path, low_memory=False)
    else:
        validation = pd.read_csv(benchmark_path, low_memory=False)
        priority = {
            "linear_t2_interpolation": 1,
            "normalized_profile_interpolation": 2,
            "one_sided_propagation_shift": 3,
            "nearest_tmc": 4,
            "corridor_median_copy": 5,
        }
        validation["_priority"] = validation["method"].map(priority).fillna(99)
        benchmark = (
            validation.sort_values(
                ["tmc", "period", "_priority"], kind="mergesort"
            )
            .drop_duplicates(["tmc", "period"], keep="first")
            .rename(columns={"predicted_t2_hour": "expansion_prediction"})
        )
        benchmark["class_prediction"] = np.nan
