"""Shared automatic process-worker planning for the NVTA workflows."""
...
def recommend_workers(
    ...
    *,
    ...
    """Use a target share of currently free logical-core capacity."""

    if not 0.0 < target_fraction <= 1.0:
        raise ValueError("target_fraction must be greater than 0 and at most 1")
    logical_cores = max(1, int(os.cpu_count() or 1))
    free_core_equivalents, measurement = _measure_free_core_equivalents(
        logical_cores,
        sample_seconds,
    )
    ...
    if explicit_workers is None:
        workers = max(
            1,
            int(math.floor(free_core_equivalents * target_fraction)),
        )
        workers = min(
            workers,
    ...
    return WorkerPlan(
        ...
        free_core_equivalents=round(free_core_equivalents, 2),
