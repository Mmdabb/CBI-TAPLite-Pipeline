...
def audit_assignment(assignment_dir: Path, resource_dir: Path) -> tuple[pd.DataFrame, dict]:
    ...
    t2["direct_observation_mapping"] = True
    ...
    for period, (period_start, period_end) in PERIODS.items():
        ...
        direct_observation_mapping = (
            matched["direct_observation_mapping"].fillna(False).astype(bool)
        )
        explicitly_closed = (
            matched.get("allowed_use", pd.Series("", index=matched.index))
            .astype("string")
            .str.strip()
            .str.lower()
            .eq("closed")
        )
        eligible_open_link = matched_link & ~explicitly_closed
        ordered = (
            t0.ge(0.0)
            & t0.lt(t2_actual)
            & t2_actual.lt(t3)
        ...
        matched["explicitly_closed"] = explicitly_closed
        matched["eligible_open_link"] = eligible_open_link
        matched["direct_observation_mapping"] = direct_observation_mapping
        matched["missing_converted_link"] = direct_observation_mapping & ~matched_link
        matched["missing_required_boundaries"] = (
            direct_observation_mapping & eligible_open_link & has_congestion & ~all_boundaries
        )
        matched["invalid_boundary_order_or_range"] = (
            direct_observation_mapping
            & eligible_open_link
            & has_congestion
            & all_boundaries
        ...
        matched["unexpected_boundaries_without_congestion"] = (
            direct_observation_mapping
            & eligible_open_link
            & ~has_congestion
            & any_boundaries
        )
        matched["missing_or_mismatched_start_speed"] = eligible_open_link & ~start_matches
        matched["missing_or_mismatched_end_speed"] = eligible_open_link & ~end_matches
        # Period-closed links may retain start/end speed anchors as boundary
        # metadata, but they must never receive observed congestion times.
        # PLF closure handling is validated by the converter itself.
        matched["closed_link_has_observed_inputs"] = (
            direct_observation_mapping & explicitly_closed & any_boundaries
        )
        violation_fields = [
            ...
            "closed_link_has_observed_inputs",
        ]
        ...
        period_summary[period.upper()] = {
            "speed_anchor_node_pairs": int(len(matched)),
            "direct_observation_node_pairs": int(direct_observation_mapping.sum()),
            "eligible_open_direct_pairs": int(
                (direct_observation_mapping & eligible_open_link).sum()
            ),
            "explicitly_closed_direct_pairs": int(
                (direct_observation_mapping & explicitly_closed).sum()
            ),
            "expected_congestion_pairs": int(
                (direct_observation_mapping & eligible_open_link & has_congestion).sum()
            ),
            "expected_no_congestion_pairs": int(
                (direct_observation_mapping & eligible_open_link & ~has_congestion).sum()
            ),
            "missing_converted_links": int(matched["missing_converted_link"].sum()),
            ...
            "closed_link_has_observed_inputs": int(
                matched["closed_link_has_observed_inputs"].sum()
            ),
