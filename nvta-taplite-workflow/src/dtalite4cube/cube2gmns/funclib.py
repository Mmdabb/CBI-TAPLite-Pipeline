...
def _assign_observed_speed_boundaries(
    ...
    *,
    ...
    # anchors are scenario inputs and remain useful for every mapped link,
    # including links closed during this period.
    if not found:
        return
...
def _loadLinks(
    ...
    vdf_type='bpr',
    observed_speed_boundary_lookup_directory=None,
    qvdf_profile_mode=2,
):
    ...
    if vdf_type == "qvdf" and observed_plf_lookup_path().is_file():
    ...
    if (
        vdf_type == "qvdf"
        and observed_speed_boundary_lookup_path(
    ...
    for row_position in range(len(network_shapefile)):
        ...
        # TAPlite's numeric QVDF selector is independent from the Cube-derived
        # link_type/vdf_code classification used to choose parameter rows.
        # Write it explicitly so the kernel always receives QVDF mode 2.
...
def _convert_network_chunk(task):
    ...
    with contextlib.redirect_stdout(captured):
        _loadLinks(
            ...
            observed_speed_boundary_lookup_directory=task[
                "observed_speed_boundary_lookup_directory"
            ],
            qvdf_profile_mode=task["qvdf_profile_mode"],
        )
        _outputLink(
            network,
            task["part_dir"],
                    ...
                    tasks.append(
                        {
                            ...
                            "vdf_type": vdf_type,
                            "observed_speed_boundary_lookup_directory": (
                                str(observed_speed_boundary_lookup_directory)
                                if observed_speed_boundary_lookup_directory is not None
                                else None
                            ),
                            "qvdf_profile_mode": int(qvdf_profile_mode),
                            "row_start": row_start,
                            "row_stop": row_stop,
                            "part_dir": part_dir,
