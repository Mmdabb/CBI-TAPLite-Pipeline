...
def apply_congestion_boundaries(
    ...
    qvdf_profile_mode: int = DEFAULT_QVDF_PROFILE_MODE,
):
    """Add period-specific observed T0/T2/T3 values to converted links."""
    if qvdf_profile_mode not in {0, 1, 2}:
    ...
    # fallback. The caller selects one valid profile mode for this scenario.
    for link in links:
        for field in LOOKUP_BOUNDARY_FIELDS:
            link.other_attrs[field] = ""
        link.other_attrs[PROFILE_MODE_FIELD] = int(qvdf_profile_mode)
    ...
    observed_t2_available = observed_path.is_file()
