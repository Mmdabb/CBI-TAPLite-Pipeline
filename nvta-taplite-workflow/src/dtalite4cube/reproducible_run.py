...
def _build_dtalite_command(kernel_source: str) -> list[str]:
    ...
    # pytaplite kernel driver. Verify the imported native binary before every
    # assignment so a same-name package from another environment cannot run.
    workflow_root = Path(__file__).resolve().parents[2]
    path_bootstrap = f"sys.path.insert(0, {str(workflow_root)!r})"
    code = f"""
import os
import sys
...
runtime_identity = verify_taplite_runtime()
print("taplite4mpo source:", taplite4mpo.__file__)
print("taplite4mpo version:", taplite4mpo.__version__)
print("taplite4mpo distribution:", runtime_identity["distribution"])
print("taplite4mpo native SHA256:", runtime_identity["native_sha256"])
