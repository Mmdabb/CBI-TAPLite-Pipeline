...
from .taplite_runtime import verify_taplite_runtime
from .resource_paths import resource_path

logger = logging.getLogger(__name__)
MAX_GLOBAL_CORES = 20
DEFAULT_QVDF_OVERRIDE_DICTIONARY = (
    resource_path("qvdf_parameter_override_lookup", "qvdf_node_pair_overrides.npy")
)
...
def _taplite_provenance() -> dict[str, object]:
    return verify_taplite_runtime()


def _qvdf_smoothing_provenance(output_root: Path) -> dict[str, object] | None:
    report_path = output_root / "qvdf_batch_report.json"
    ...
    return {
        ...
        "write_back": report.get("write_back"),
    }


def _qvdf_override_provenance(output_root: Path) -> dict[str, object] | None:
    manifest_path = output_root / "qvdf_override_audit" / "qvdf_override_manifest.json"
    ...
    *,
    ...
    manifest = {
        ...
        "settings": {
            ...
            "observed_speed_boundary_lookup_directory": (
                str(config.observed_speed_boundary_lookup_directory.resolve())
                if config.observed_speed_boundary_lookup_directory is not None
                else None
            ),
            "qvdf_profile_mode": config.qvdf_profile_mode,
            ...
            "qvdf_smoothing": {
                ...
                "workers": config.qvdf_smoothing_workers or config.processors,
                "backup": config.qvdf_smoothing_backup,
            },
            "demand_output_format": config.demand_output_format,
        },
        "qvdf_smoothing": _qvdf_smoothing_provenance(output_root),
        "qvdf_parameter_override": _qvdf_override_provenance(output_root),
        "periods": periods,
    }
...
class AssignmentConfig:
    ...
    observed_speed_boundary_lookup_directory: Path | None = None
    qvdf_profile_mode: int = 2
    qvdf_parameter_override: bool = False
    qvdf_override_dictionary: Path | None = None
    qvdf_smoothing: bool = True
    qvdf_smoothing_workers: int = 0
    qvdf_smoothing_backup: bool = True
    ...
    def validate(self) -> None:
        ...
        if self.qvdf_profile_mode not in {0, 1, 2}:
            raise ValueError("qvdf_profile_mode must be 0, 1, or 2.")
        ...
        if self.qvdf_smoothing_workers < 0:
            raise ValueError("qvdf_smoothing_workers must be zero (use assignment processors) or positive.")
        if self.qvdf_smoothing_workers > 0:
            self.qvdf_smoothing_workers = min(
                int(self.qvdf_smoothing_workers), MAX_GLOBAL_CORES
            )
    ...
    def resolved_qvdf_override_dictionary(self) -> Path:
        if self.qvdf_override_dictionary is not None:
            return self.qvdf_override_dictionary.resolve()
        return DEFAULT_QVDF_OVERRIDE_DICTIONARY
...
def run_network_conversion(config: AssignmentConfig) -> dict:
    ...
    result = get_gmns_from_cube(
        ...
        observed_speed_boundary_lookup_directory=(
            config.observed_speed_boundary_lookup_directory
        ),
        qvdf_profile_mode=config.qvdf_profile_mode,
    )
    logger.info(
...
def resolve_run_folder(config: AssignmentConfig, time_period: str | None = None) -> Path:
    ...
    if configured_path is not None:
        ...
        base = config.network_path / "dtalite_runs" / "run"
...
def run_reproducible_dtalite(
    *,
    ...
    # A successful assignment supersedes a failure marker left by an earlier
    # resume attempt in the same prepared period folder.
    (run_folder / "RUN_FAILURE.json").unlink(missing_ok=True)
...
def run_assignment_pipeline(config: AssignmentConfig) -> bool:
    ...
    if config.network_conversion:
        ...
        if config.qvdf_parameter_override:
            ...
            logger.info(
                "Applying post-mapping QVDF node-pair overrides from %s",
                config.resolved_qvdf_override_dictionary,
            )
            conversion_profile["stages"]["qvdf_parameter_override"] = apply_overrides(
                scenario_output_dir,
                config.resolved_qvdf_override_dictionary,
                override_audit_root / "pre-overlay-network",
                expected_profile_mode=config.qvdf_profile_mode,
                periods=config.active_time_periods,
            )
    ...
    if config.qvdf_smoothing and not config.dry_run:
        from ..qvdf_speed_smoother import smooth_assignment_outputs

        smoother_workers = min(
            config.qvdf_smoothing_workers or config.processors,
            MAX_GLOBAL_CORES,
        )
        logger.info(
            "Smoothing QVDF speed profiles after assignment: periods=%s workers=%s",
            config.active_time_periods,
            smoother_workers,
        )
        smoothing_report = smooth_assignment_outputs(
            scenario_output_dir,
            [normalize_period_key(period) for period in config.active_time_periods],
            workers=smoother_workers,
            backup=config.qvdf_smoothing_backup,
            report_path=scenario_output_dir / "qvdf_batch_report.json",
        )
        if smoothing_report.get("status") != "complete":
            raise RuntimeError(
                "QVDF speed smoother did not complete successfully: "
                f"{smoothing_report.get('status')}"
            )
        logger.info("QVDF speed smoothing complete")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--observed-speed-boundary-lookup-directory", type=Path)
    parser.add_argument("--qvdf-profile-mode", type=int, choices=[0, 1, 2], default=2)
    parser.add_argument("--qvdf-parameter-override", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--qvdf-override-dictionary", type=Path)
    parser.add_argument("--qvdf-smoothing", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--qvdf-smoothing-workers", type=int, default=0)
    parser.add_argument("--qvdf-smoothing-backup", action=argparse.BooleanOptionalAction, default=True)
...
def main() -> None:
    ...
    config = AssignmentConfig(
        ...
        observed_speed_boundary_lookup_directory=(
            args.observed_speed_boundary_lookup_directory
        ),
        ...
        qvdf_smoothing_workers=args.qvdf_smoothing_workers,
        qvdf_smoothing_backup=args.qvdf_smoothing_backup,
