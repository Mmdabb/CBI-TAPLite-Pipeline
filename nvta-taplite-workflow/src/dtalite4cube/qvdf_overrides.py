...
def apply_overrides(
    ...
    *,
    ...
    selected_periods = tuple(
        dict.fromkeys(period.lower() for period in (periods or PERIOD_SEQUENCE))
    )
    unknown = sorted(set(selected_periods).difference(PERIOD_SEQUENCE))
    if unknown:
        raise ValueError(f"QVDF override dictionary has no period sequence for: {unknown}")
...
def main() -> int:
    ...
    parser.add_argument("--periods", nargs="+", default=list(PERIOD_SEQUENCE))
