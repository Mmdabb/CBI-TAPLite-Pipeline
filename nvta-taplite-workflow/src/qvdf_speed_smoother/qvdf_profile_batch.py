...
# sys.path.  Add the root so both ``python tools/...py`` and ``python -m tools...``
# use the same imports (spawned Windows workers also import this module safely).
if __package__ in (None, ""):
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

from .qvdf_profile_experiment import (
    ConnectorLimits,
    QvdfInputs,
    _constraint_metrics,
...
def _process_chunk(task: dict[str, object]) -> dict[str, object]:
    ...
    connection = sqlite3.connect(
        f"file:{Path(db_path).as_posix()}?mode=ro", uri=True, timeout=120.0
    )
...
def run_batch(args: argparse.Namespace) -> dict[str, object]:
    ...
    # On Windows, tempfile.mkdtemp() may create a directory with a private
    # security descriptor that a subsequently opened SQLite handle cannot
    # traverse under an impersonated/managed runtime.  Create the stage with
    # normal inherited workspace permissions instead.  The run id and PID
    # keep the directory collision-safe while preserving the existing cleanup
    # behavior below.
    if os.name == "nt":
        stage_dir = scenario_dir / f".qvdf-batch-{run_id}-{os.getpid()}"
        stage_dir.mkdir(parents=False, exist_ok=False)
    else:
        stage_dir = Path(
            tempfile.mkdtemp(prefix=".qvdf-batch-", dir=scenario_dir)
        )
