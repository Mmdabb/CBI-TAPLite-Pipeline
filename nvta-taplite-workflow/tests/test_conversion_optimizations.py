...
from src.dtalite4cube.dtab import demand_binary_path, inspect_dtab, write_dtab_file
...
from src.dtalite4cube.omx2csv import _close_worker_omx_files, _worker_omx_file
...
class DirectAssignmentRoutingTests(WorkspaceArtifactTestCase):
    def test_pipeline_runs_each_prepared_period_folder_directly(self):
        ...
        config = AssignmentConfig(
            ...
            qvdf_smoothing=False,
        )

        with (
            patch(
                "src.dtalite4cube.runner.prepare_dtalite_period_folders",
                return_value={"am": period_dir},
            ),
            patch("src.dtalite4cube.runner.remove_root_period_duplicates"),
            patch(
                "src.dtalite4cube.runner.run_period_readiness_check",
                return_value=readiness,
            ),
            patch(
                "src.dtalite4cube.runner.run_reproducible_dtalite",
                return_value=period_dir,
            ) as direct_run,
            patch("src.dtalite4cube.runner._write_pipeline_manifest"),
        ):
