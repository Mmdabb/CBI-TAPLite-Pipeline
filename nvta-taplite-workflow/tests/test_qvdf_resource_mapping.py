...
from src.dtalite4cube.cube2gmns.fieldnameconfig import qvdf_params
...
from src.dtalite4cube.resources.observed_link_plf_lookup.load_observed_link_plf import (
    lookup as lookup_observed_plf,
)
from src.dtalite4cube.resources.observed_link_speed_boundary_lookup.load_observed_link_speed_boundaries import (
    lookup as lookup_observed_speed_boundaries,
)
from src.dtalite4cube.resources.observed_link_t2_lookup.load_observed_link_t2 import (
...
class QvdfResourceMappingTests(unittest.TestCase):
    def test_closed_link_skips_plf_but_retains_speed_anchor_overrides(self):
        class Link:
            def __init__(self):
                self.other_attrs = {}
    ...
    def test_qvdf_lookup_comes_from_packaged_resource_csv(self):
        self.assertEqual(
            QVDF_RESOURCE_PATH,
            WORKFLOW_ROOT / "src" / "dtalite4cube" / "resources" / "link_qvdf.csv",
        )
    ...
    def test_missing_period_field_fails_instead_of_silently_defaulting(self):
        mapping = get_vdf_dict("qvdf")
        selected_key, record = _select_vdf_record(mapping, 101)

        with self.assertRaisesRegex(ValueError, "QVDF_plf4"):
            _vdf_parameter_value(
                record,
                vdf_type="qvdf",
                vdf_field="plf",
